...
def get_selected_foods(nutrient_keyword, nutrient_table, items_number, basic_food):
    ...
    if basic_food in top_items:
        top_items = sorted_nutrient_table.head(items_number)['Item'].tolist()
        return top_items
    else:
...
def total_nutrient_in_recommendations(nutrient_table, nutrient_keyword):
    ...
    total_nutrient_value = round(nutrient_table[nutrient_column].sum(), 2)
